...
typedef struct {
    int transitions[DFA_NSYMBOLS]; //value of transition is which state to go, where the char is the
    int is_accepting;				//equivalent position in the array
} DFA_State;
...
extern void DFA_set_transition_remaining(DFA *dfa, int src, int dst);
...
extern int DFA_execute_from_one(DFA *dfa, char *input);
